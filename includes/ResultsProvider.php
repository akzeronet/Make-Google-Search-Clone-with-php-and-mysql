...
class ResultsProvider {
    ...
    public function get_total_result($type, $term) {
        ...
        if ($type == 'videos') {
            return $this->total_videos_result($term);
        }
    }
    ...
    public function total_videos_result($term) {
        $query = $this->db->prepare("SELECT COUNT(*) as total 
                                         FROM videos 
                                         WHERE title LIKE :term");
    ...
    public function videos_html_result($page, $pageSize, $term) {
        $fromLimit = ($page - 1) * $pageSize;
        $query = $this->db->prepare("SELECT * 
                                         FROM videos 
                                         WHERE title LIKE :term
        ...
        while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
            ...
            $resultsHtml .= "<div class='gridItem video'>
